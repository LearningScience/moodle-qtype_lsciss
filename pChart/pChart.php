     ...
     if ( isset($DataDescription["Axis"]["Y"]) )
      {
       ...
       $TextTop    = (($this->GArea_Y2 - $this->GArea_Y1) / 2) + $this->GArea_Y1 + ($TextHeight/2);
       //imagettftext($this->Picture,$this->FontSize,90,$XMin-$this->FontSize,$TextTop,$C_TextColor,$this->FontName,$DataDescription["Axis"]["Y"]);
       $this->drawYaxisTitle($DataDescription["Axis"]["Y"],$C_TextColor);
      }
     ...
     if ( isset($DataDescription["Axis"]["X"]) )
      {
       ...
       $TextLeft   = (($this->GArea_X2 - $this->GArea_X1) / 2) + $this->GArea_X1 + ($TextWidth/2);
       //imagettftext($this->Picture,$this->FontSize,0,$TextLeft,$YMax+$this->FontSize+5,$C_TextColor,$this->FontName,$DataDescription["Axis"]["X"]);
       $this->drawXaxisTitle($DataDescription["Axis"]["X"],$C_TextColor);
      }
    }

    
    function drawXaxisTitle($axistitle,$color)
    {
        //Learning Science Add on to hand subscript and superscripts
       $angle = 0;
       $Position = imageftbbox($this->FontSize, $angle, $this->FontName, $axistitle);
       $TextWidth = abs($Position[2]) - abs($Position[0]);
       $TextHeight = abs($Position[7]) - abs($Position[1]);
       
       $graphwidth = $this->GArea_X2 - $this->GArea_X1;
       $graphheight = $this->GArea_Y2 - $this->GArea_Y1;
        
       $x = $graphwidth - $this->GArea_X1 - ($TextWidth / 2);
       $y = $graphheight + $this->GArea_Y1 + ($TextHeight *4);
            
       imagettftext($this->Picture,$this->FontSize,$angle,$x,$y,$color,$this->FontName,$axistitle); 
    }
    
    function drawYaxisTitle($axistitle,$color)
    {
       ...
       $TextHeight = abs($Position[6]) - abs($Position[0]);
       
       $graphwidth = $this->GArea_X2 - $this->GArea_X1;
       $graphheight = $this->GArea_Y2 - $this->GArea_Y1;
             
       $x = 0;//$TextHeight*2;
       $y = 0;//($graphheight /2) + ($TextWidth/1.2);
       $x = $TextHeight*2;
       $y = ($graphheight /2) + ($TextWidth/1.2);
       imagettftext($this->Picture, $this->FontSize, $angle, $x, $y, $color, $this->FontName, $axistitle);
      // $axistitle = "<sup>y</sup> axis title <sup>2</sup> and <sub>this</sub><sup>and another one</sup> yet more text!!!!";

       preg_match_all("/<sup>(.*?)<\/sup>/", $axistitle, $superscript_matches);
       preg_match_all("/<sub>(.*?)<\/sub>/", $axistitle, $subscript_matches);
      
    
       $tagpos = array();
       
        for($i=0;$i<=count($superscript_matches[0]);$i++)
           {
            @$pos = strpos($axistitle,$superscript_matches[0][$i]);

            if($pos !== false){
                @$tagpos[$pos]->text = $superscript_matches[1][$i];
                $tagpos[$pos]->startposition = $pos;
                $tagpos[$pos]->endposition = $pos + strlen($axistitle);
                $tagpos[$pos]->font = dirname(__FILE__) . '/../' . 'Fonts/superscript.ttf';
                $tagpos[$pos]->matchlength = strlen($superscript_matches[0][$i]);
                $tagpos[$pos]->fontsize = $this->FontSize*1.4;
            }
            
            }

          for($i=0;$i<=count($subscript_matches[0]);$i++)
           {
              @$pos = strpos($axistitle,$subscript_matches[0][$i]);
             
              if($pos !== false){
                  @$tagpos[$pos]->text = $subscript_matches[1][$i];
                  $tagpos[$pos]->startposition = $pos;
                  $tagpos[$pos]->endposition = $pos + strlen($axistitle);
                  $tagpos[$pos]->matchlength = strlen($subscript_matches[0][$i]);
                  $tagpos[$pos]->font = dirname(__FILE__) . '/../' . 'Fonts/subscript.ttf';
                  $tagpos[$pos]->fontsize = $this->FontSize*1.4;
              }
            }  
            
            $tagposkeys = array_keys($tagpos);
            
            sort($tagposkeys);

            $currentpos = 0;
            
            $axistitlearray = array();

            foreach($tagposkeys as $key)
                {
                $len = $tagpos[$key]->startposition - $currentpos;
//                 error_log("len: ".print_r($len,true));
//                  error_log("currentpos start: ".print_r($currentpos,true));
 
                $text = substr($axistitle,$currentpos,$len);
// error_log("text");
// error_log(print_r($text,true));
 
                if(trim($text)!= "")
                    {
                    $axistitlearray[$currentpos]->text = $text;
                    $axistitlearray[$currentpos]->font = $this->FontName;
                    $axistitlearray[$currentpos]->fontsize = $this->FontSize;
//                    error_log("axistitlearray :".print_r($currentpos,true));
//                error_log(print_r($axistitlearray[$currentpos],true));
                    $currentpos += strlen($text);
                    }


                @$axistitlearray[$key]->font = $tagpos[$key]->font;
                $axistitlearray[$key]->fontsize = $tagpos[$key]->fontsize;
                $axistitlearray[$key]->text = $tagpos[$key]->text;
                
                $currentpos += $tagpos[$key]->matchlength;
//                error_log("axistitlearray :".print_r($key,true));
//                error_log(print_r($axistitlearray[$key],true));
//                error_log("currentpos end: ".print_r($currentpos,true));
                }
            
              if($currentpos !== strlen($axistitle))
              {
                  $text = substr($axistitle,$currentpos);
              ...
              $y=$graphheight;
              //$y=0;
              foreach($axistitlearray as $key => $value)
                  {
                  $axistitlearray[$key]->position = imageftbbox($axistitlearray[$key]->fontsize, $angle, $axistitlearray[$key]->font,$axistitlearray[$key]->text);
                  $axistitlearray[$key]->textwidth = abs($axistitlearray[$key]->position[3]) - abs($axistitlearray[$key]->position[1]);
                  $axistitlearray[$key]->textheight = abs($axistitlearray[$key]->position[6]) - abs($axistitlearray[$key]->position[0]);
                  ...
                  if ($angle == 90)
                      ...
                      $y -= $axistitlearray[$key]->textwidth;
                      $x = $axistitlearray[$key]->textheight*2;
                      }
                  else
                      {
                      $x += $axistitlearray[$key]->textwidth;
                      $y = $graphheight - $axistitlearray[$key]->textheight;
                      }
                      
                  $axistitlearray[$key]->y = $y;
                  $axistitlearray[$key]->x = $x;
                  }
                                    
                  //error_log("axistitlearray");
                //error_log(print_r($axistitlearray,true));
                //$this->drawAxistitleArray($axistitlearray,$angle,$color);
                
//                  $temp = array_keys($axistitlearray);
//                  $blip = $axistitlearray[$temp[0]];
//imagettftext($this->Picture, $this->FontSize, $angle, $x, $y, $color, $blip->font, $blip->text);

       //imagettftext($this->Picture, $this->FontSize, $angle, $x, $y, $color, $this->FontName, $axistitle);
        }
        
        function drawAxistitleArray($axistitlearray,$angle,$color)
            {
            foreach($axistitlearray as $axischunk)
                {
                imagettftext($this->Picture, $axischunk->fontsize, $angle, $axischunk->x, $axischunk->y, $color, $axischunk->font, $axischunk->text);
                }
            }
   ...
   function drawTreshold($Value,$R,$G,$B,$ShowLabel=FALSE,$ShowOnRight=FALSE,$TickWidth=4,$FreeText=NULL)
    ...
    function drawLineOfBestFit($x1,$y1,$x2,$y2)
    {
        /**
         * Learning Science mod, this lib will not be updated until version 2 so its been modified!
         * this function draws a line based on $x1,$y1 and $x2,$y2, see drawtreshold function above :)
         */
        $ShowLabel=FALSE;
        $ShowOnRight=FALSE;
        $TickWidth=4;
     ...
     $Y1 = $this->GArea_Y2 - ($y1 - $this->VMin) * $this->DivisionRatio;
     $Y2 = $this->GArea_Y2 - ($y2 - $this->VMin) * $this->DivisionRatio;
     $X1 = $this->GArea_X1 + (($x1 - $this->VXMin) * $this->XDivisionRatio);
     $X2 = $this->GArea_X1 + (($x2 - $this->VXMin) * $this->XDivisionRatio);
     ...
     if ( $TickWidth == 0 ){
      ...
      $this->drawDottedLine($X1,$Y1,$X2,$Y2,$TickWidth,$R,$G,$B);
     }
     ...
     if ( $ShowLabel )
      {
       ...
       if ( $ShowOnRight )
        imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X2+2,$Y1+($this->FontSize/2),$C_TextColor,$this->FontName,$Label);
       else
        imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X1+2,$Y1-($this->FontSize/2),$C_TextColor,$this->FontName,$Label);
      }
    }
  ...
  /* This function draw a plot graph in an X/Y space */
  function drawXYPointsGraph($xseries, $yseries, $DataDescription, $YSerieName, $XSerieName, $PaletteID=0, $BigRadius=5, $SmallRadius=2, $R2=-1, $G2=-1, $B2=-1, $Shadow=TRUE)
        {
        /**
         * Learning Science mod to draw a scatter graph without point (0,0)
         * Function to draw a proper scatter graph, borrowed from XYPlotGraph above
         */
        $R = $this->Palette[$PaletteID]["R"];
        $G = $this->Palette[$PaletteID]["G"];
        $B = $this->Palette[$PaletteID]["B"];
        ...
        foreach ($xseries as $key => $xvalue)
            {

            if($xseries[$key]==="" || $yseries[$key]==="" || $xseries[$key]===NULL || $yseries[$key]===NULL )
                {
                continue;
            }
            $X = $xseries[$key];
            ...
            $this->drawFilledCircle($X, $Y, 4, 255, 50, 0);                    
            }
